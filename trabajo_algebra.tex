...
\newcommand{\bigtitle}[1]{\par\addcontentsline{toc}{section}{#1}\begin{center}\normalfont\Huge\bfseries #1\end{center}\par}
\newcommand{\medtitle}[1]{\par\addcontentsline{toc}{subsection}{#1}\begin{center}\normalfont\Large\bfseries #1\end{center}\par}
\newcommand{\smalltitle}[1]{{\par\addcontentsline{toc}{subsubsection}{#1}\noindent\normalfont\large\bfseries #1\par}}

{\normalfont\rmfamily
\begin{center}
\vspace*{4cm}
{\Large\bfseries CONEXIÓN INTERNA BLUETOOTH}
\end{center}
\vfill
\begin{center}


Adrián Sánchez Torres \\[0.4em]
Daniel Aparisi Lozano \\[0.4em]
Diego Sola Novoa \\[0.4em]
Javier Meneses Rode
\end{center}
\vspace*{2cm}
}
...
{\normalfont\rmfamily
\begin{center}\Huge\bfseries ÍNDICE\end{center}
\vspace{0.5em}
\tableofcontents
}
...
$A$ es una matriz que representa el sistema de modulación,
...
puede representar $2^k$ mensajes distintos (en el caso binario, claro).
...
$B$ la matriz del canal, que representa cómo el medio de transmisión transforma la señal
...
multitrayectoria, fenómenos que se modelan dentro de $B$. Cada elemento de $B$ describe cómo una
...
Además de la transformación producida por el canal ($B$), está el ruido ($n$), que se añade al producto
de $Bx$. Por eso, aun conociendo perfectamente el canal $B$, $y$ nunca coincidiría exactamente con
$Bx$. Se asume que el ruido es un vector aleatorio que sigue una distribución normal, lo que se
...
encontrar el vector cuya transformación $Bx$ esté más cerca del vector $y$ en norma euclídea.
...
propiedades algebraicas de la matriz del canal. El rango de la matriz $B$ indica cuánta información
...
total de ruido, sería posible recuperar $x$ calculando la inversa: \centerline{$x=B^{-1}y$}
...
Cuando se modela un canal inalámbrico mediante matrices, los autovectores de $B$ representan
...
$B$, mientras que el autovalor indica cuánto se amplifica la señal en esa dirección.
...
$B\in\mathbb{R}^{3\times 3}$ es la matriz del canal,
...
\newpage
\smalltitle{3.2.6 Corrección mediante proyección en subespacios}
...
el proceso de codificación y corrección de errores descrito en este apartado es correcto.
...
Este trabajo nos ha permitido conocer y analizar los fundamentos matemáticos que forman y dan lugar a 
el estándar de comunicación inalámbrica Bluetooth a partir de bases matemáticas. 

A lo largo del desarrollo, se ha podido ver cómo la información digital y las señales de radio pueden 
ser modeladas con precisión mediante vectores en espacios multidimensionales y cuerpos finitos. En primer
lugar, se ha detallado cómo estas señales se estructuran y protegen antes de su emisión, empleando
matrices generadoras y de control de paridad para introducir redundancia y anticiparse a los errores de transmisión.
Posteriormente, se ha abordado la naturaleza física de la radiofrecuencia mediante la modelización del canal
inalámbrico; aquí, el uso de matrices de transformación ha permitido abstraer los efectos del ruido y la
multitrayectoria, demostrando que el receptor logra recuperar el mensaje original calculando proyecciones 
ortogonales sobre subespacios de códigos válidos. Finalmente, el análisis de los protocolos de seguridad ha
revelado que la protección de los datos frente a interceptaciones externas depende directamente de operaciones 
sobre cuerpos de Galois, donde la robustez del cifrado E0 se sostiene sobre la complejidad de resolver sistemas masivos
de ecuaciones lineales booleanas. 

Hoy en día la tecnología Bluethooth es una de las tecnologías más usadas a nivel mundial que sigue evolucionando 
progresivamente, uno de los objetivos es poder reforzar la seguridad de las claves incluyendo algoritmos 
criptográficos-cuánticos lo que dará lugar a ecuaciones matemáticas más complejas y sofisticadas que requerirán
de un conocimiento profundo del álgebra para su desarrollo y comprensión.  

Este trabajo es un claro ejemplo de lo importante y fundamental que es el álgebra, que ofrece
un lenguaje universal y carece de ambigüedad proporcionando así las bases de cualquier rama tecnológica. 
\newpage
\bigtitle{5. Referencias}
Branch Education. (2021). \textit{How does Bluetooth work?} [Video]. YouTube. \url{https://youtu.be/1I1vxu5qIUM?si=p8DfM2lics0ytzYJ}

Gallager, R. G. (2008). \textit{Principles of digital communication}. Cambridge University Press.

Goldsmith, A. (2005). \textit{Wireless communications}. Cambridge University Press.

Haykin, S. (2002). \textit{Adaptive filter theory} (4th ed.). Prentice Hall.

IACR ePrint Archive. (2022). \textit{An algebraic attack to the Bluetooth stream cipher E0} (Report 2022/016). \url{https://eprint.iacr.org/2022/016.pdf}

Lay, D. C. (2012). \textit{Linear algebra and its applications}. Pearson.

Lay, D. C., Lay, S. R., \& McDonald, J. J. (2015). \textit{Linear algebra and its applications} (5th ed.). Pearson.

Lin, S., \& Costello, D. J. (2004). \textit{Error control coding} (2nd ed.). Pearson.

Oppenheim, A. V., \& Schafer, R. W. (2010). \textit{Signals and systems} (2nd ed.). Prentice Hall.

\textit{Parity-check matrix}. (n.d.). ScienceDirect. \url{https://www.sciencedirect.com/topics/computer-science/parity-check-matrix}

Proakis, J. G. (2001). \textit{Digital communications} (4th ed.). McGraw-Hill.

Strang, G. (2016). \textit{Introduction to linear algebra} (5th ed.). Wellesley-Cambridge Press.

\textit{The confidentiality-integrity-accessibility triad into the knowledge security: A reassessment from the point of view of the knowledge contribution to innovation}. (n.d.). ResearchGate. \url{https://www.researchgate.net/publication/257985911_The_Confidentiality_-_Integrity_-_Accessibility_Triad_into_the_Knowledge_Security_A_Reassessment_from_the_Point_of_View_of_the_Knowledge_Contribution_to_Innovation}
